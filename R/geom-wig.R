GeomWig <- ggproto("GeomWig", Geom,
                    required_aes = c("x", "height", "transcripts"),
                    #' designed for plot RNA-Seq data. transcripts
                    non_missing_aes = c("linewidth", "shape"),
                    extra_params = c("exon_height", "na.rm", "x_translation", "layout"),
                    ...
                    setup_data = function(data, params){
                      #' in the setting up step, which at least needed two dataframes. one is gff file and
                      #' another file is wiggle like file or wiggle-like data.
...
geom_wig <- function(mapping = NULL, data = NULL,
                      stat = "identity", position = "identity",
                      ..., na.rm = FALSE, show.legend = NA, exon_height=1.5,
                      x_translation = NULL,layout = "up",
                      inherit.aes = TRUE) {
  layer(
    data = data,
    mapping = mapping,
