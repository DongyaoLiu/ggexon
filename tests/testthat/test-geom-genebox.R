...
test_that("linked codons must match the transcript before becoming explicit anchors", {
  ...
  S4Vectors::mcols(features) <- S4Vectors::DataFrame(
    type = c(
      "mRNA", "CDS", "start_codon", "stop_codon", "stop_codon",
      "mRNA", "CDS", "start_codon", "stop_codon", "stop_codon",
      "mRNA", "CDS", "start_codon", "stop_codon"
    ),
    transcript_id = c(
      rep("tx_seqname", 5L),
      rep("tx_strand", 5L),
  ...
  compiled <- .genebox_compile_coding_transcripts(
    features,
    anchor = "middle",
    track = "test",
    individual = "test",
  ...
  compiled <- compiled[
    match(c("tx_seqname", "tx_strand", "tx_bounds"), compiled$transcript_id),
    ,
    drop = FALSE
  ]

  expect_equal(compiled$anchor_start, c(121, 421, 721))
  expect_equal(compiled$anchor_end, c(179, 479, 779))
  expect_equal(compiled$coding_xmin, c(120, 420, 720))
  expect_equal(compiled$coding_xmax, c(180, 480, 780))
  expect_equal(
    compiled$initiation_anchor_source,
    c(
      "terminal_CDS_positional_proxy",
      "explicit_start_codon",
      "explicit_start_codon"
  ...
  expect_equal(compiled$initiation_anchor_fallback, c(TRUE, FALSE, FALSE))
  expect_equal(compiled$stop_anchor_fallback, c(FALSE, TRUE, TRUE))
  expect_true(all(compiled$any_anchor_fallback))
})
