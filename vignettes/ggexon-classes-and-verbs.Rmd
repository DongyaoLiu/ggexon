...
`SynBigWigAnnotation` keeps the BigWig path as an attached annotation layer;
the signal is queried only when a genomic region is requested.  This keeps
queries immutable: asking for signal returns ranges and does not modify the
attached object.
...
public `track` label while remaining separate panels. Coverage panels are
placed above the existing annotation/link chain, inherit ordinary genomic x
windows from an annotation source when one exists, and are never used as link
source panels. A standalone coverage panel can instead use its own coordinates
or a named `facet_genomics(xlim = ..., xlim_chr = ...)` window.

The layout's `SCALE_Y` column records scale-object inheritance. Panels with the
same `SCALE_Y` value train through one y-scale object; different values mean
independent objects. `scale_panel_annotation()` and `scale_panel_coverage()`
control those identities by role:

- annotation defaults to `"fixed_y"`, so annotation panels share one scale;
...
`center_panel_annotation()` changes only the post-training annotation view in
a Syn-backed plot. It does not alter annotation rows, inherited scales,
coverage/link panels, or ggplot2 position adjustment. It is the dedicated
Syn-aware equivalent of `facet_genomics(vertical = "center")`. Ordinary
non-Syn faceting and `facet_genomictree()` keep their existing behavior. See the
`bigwig-coverage-demo` article for a complete runnable example with bundled
files.
...
For an attached BigWig layer, `query_annotation()` is the generic
region-query interface and takes a length-one `GRanges`.  `query_signal()` is
the typed shortcut that accepts chromosome and coordinates.  Both calls return
the same interval-native signal ranges with raw `score` metadata; they do not
alter the annotation object or expand intervals to per-base rows.
...
Other data verbs include `query_annotation()`, `query_variants()`,
`query_signal()`, `query_domains()`, `query_protein_mutations()`, `extract_cds_seq()`,
