...
#### Role-qualified panels and inherited y scales

Syn-aware layouts distinguish annotation, coverage, and link panels by
`panel_type`. A coverage panel and an annotation panel may keep the same
public `track` label while remaining separate panels. Coverage panels
are placed above the existing annotation/link chain, inherit ordinary
genomic x windows from an annotation source when one exists, and are
never used as link source panels. A standalone coverage panel can
instead use its own coordinates or a named
`facet_genomics(xlim = ..., xlim_chr = ...)` window.

The layout’s `SCALE_Y` column records scale-object inheritance. Panels
with the same `SCALE_Y` value train through one y-scale object;
different values mean independent objects.
...
control those identities by role:

- annotation defaults to `"fixed_y"`, so annotation panels share one
  scale;
- coverage follows the y part of `facet_genomics(scales = ...)` when no
  coverage wrapper is supplied;
- an explicit role wrapper takes precedence over that facet fallback;
- a valid wrapper for a role absent from the plot is a no-op.
...
changes only the post-training annotation view in a Syn-backed plot. It
does not alter annotation rows, inherited scales, coverage/link panels,
or ggplot2 position adjustment. It is the dedicated Syn-aware equivalent
of `facet_genomics(vertical = "center")`. Ordinary non-Syn faceting and
[`facet_genomictree()`](https://dongyaoliu.github.io/ggexon/reference/facet_genomictree.md)
keep their existing behavior. See the `bigwig-coverage-demo` article for
a complete runnable example with bundled files.
